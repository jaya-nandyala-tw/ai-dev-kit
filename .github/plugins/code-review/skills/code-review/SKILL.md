...
- When changes span multiple repos/services
...
Constitution reference (for direct reads or to pass as context to Explore):
<!-- TEMPLATE: point these at your own per-area convention docs -->
...
- UI/JS changes: targeted `npm test` (or typecheck/lint available in repo)
- Terraform changes: `terraform fmt -check -recursive` + `terraform validate`

<!-- TEMPLATE: replace/extend the above with your own stack's smallest-relevant-check commands. -->
