...
4. Scope classification: **new feature/epic, tech design/spike, independent story, enhancement, or
   bug fix/hotfix** — this 5-way question is asked automatically by `@intake` before anything else,
...
If the work is larger than one story, treat it as a feature with multiple stories and plan dependency order first — and if it sits under an Epic, say so up front: intake will check `plans/epics/{EPIC-KEY}.md` for decisions already made by sibling stories under the same epic (see § Epics & Related Stories below).
...
`@intake` — you'll see it run classification, ticket fetch, spec loading, and interrogation before a plan
...
What `@intake` (via `@story`) should do:
...
If a story is part of an Epic, `@intake` fetches `epic_key`/`epic_name` from your tracker and checks for
...
intake continues; if it doesn't exist yet and this is new-feature/epic work, `@intake` offers to
...
2. Ensure relevant specs are updated — if any task touched a `specs/` file, `@story` invokes
   `@doc-sync` automatically for a gap-check before archiving; you'll still be asked before it
